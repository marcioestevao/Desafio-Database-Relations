...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (!customerExists) {
      throw new AppError('Could not find any customer wit the givem id');
    }
    ...
    if (findProductsWithNoQuantityAvaiable.length) {
      throw new AppError(
        `The quantity ${findProductsWithNoQuantityAvaiable[0].quantity} is not avaliable for ${findProductsWithNoQuantityAvaiable[0].id}`,
      );
    }
